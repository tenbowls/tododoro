...
try: 
    while True: 
        ...
        row_dict["start_time"] = oh.get_datetime_now()
        if tm.timer(time):
            # Add to database for successfully completed timer (i.e. not quit)
            row_dict["end_time"] = oh.get_datetime_now()
            row_dict["duration"] = time * 60
            row_dict["timer_category"] = "focus" if focus else "break"
